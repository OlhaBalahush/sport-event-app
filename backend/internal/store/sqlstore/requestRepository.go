...
// Update updates an existing request record
func (rr *RequestRepository) CheckByUserID(id string) (bool, error) {
	// Query the requests table to check if the user sent request
	query := `SELECT EXISTS(SELECT 1 FROM requests WHERE user_id = $1)`
	var exists bool
	err := rr.store.db.QueryRow(query, id).Scan(&exists)
	if err != nil {
		return false, err
	}
	return exists, nil
